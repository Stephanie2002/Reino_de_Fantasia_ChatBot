...
export function Chatbot() {
    ...
    setMessages([
      {
        id: "welcome",
        role: "assistant",
        content: "¡Saludos, viajero! Soy el Bibliotecario Real del Reino de Fantasía. ¿En qué puedo ayudarte hoy? 📚✨",
        timestamp: new Date(),
      },
    ])
  }, [])
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // 2. Uso de POST y manejo de errores de red
  ...
  const formatTime = (date: Date) => {
    if (!mounted) return "" // Evita discrepancia de tiempo en SSR
  ...
  // Si no ha montado en el cliente, no renderizamos nada para evitar el error #418
      ...
      >
        ...
        <div className="flex items-center gap-3 border-b border-border bg-primary px-4 py-4">
          <div className="flex-1">
            <h3 className="font-serif text-lg font-semibold text-primary-foreground">ChatBot</h3>
            <p className="text-xs text-primary-foreground/80">Pregunta por el Reino de Fantasía</p>
          </div>
          <button
        ...
        {/* Formulario */}
        <form onSubmit={handleSendMessage} className="flex items-center gap-2 border-t border-border bg-card p-4">
          <input
            ref={inputRef}
            type="text"
            value={inputValue}
        ...
        <div className="border-t border-border bg-muted/50 px-4 py-3">
          ...
          <div className="flex flex-wrap gap-2">
            {["Nuestros Libros", "Autores"].map((suggestion) => (
              <button
                key={suggestion}
